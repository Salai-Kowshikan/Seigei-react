...
export default function Navbar({ aboutRef }) {
    ...
    return (
        <div className="sticky top-0 z-50 bg-accent flex justify-between items-center max-w-full">
            ...
            <div className="mobile:hidden relative">
                <button onClick={() => setDropdownOpen(!dropdownOpen)} className="text-black font-inter px-2 font-bold text-[24px] bg-inherit border-none cursor-pointer">
                    <img src="/Menu.svg" alt="Menu" />
                </button>
                {dropdownOpen && (
                    <div className="absolute right-0 w-48 my-4 mx-2 rounded-md bg-accent ring-1 ring-black ring-opacity-5">
                        <div className="py-1 flex flex-col" role="menu" aria-orientation="vertical" aria-labelledby="options-menu">
                            <button onClick={() => window.open('https://indiansignlanguage.org/', '_blank')} className="text-black font-inter font-bold text-xl border-0 border-b-2 border-black border-solid bg-inherit cursor-pointer">ISL</button>
                            <button className="text-black font-inter font-bold text-xl bg-inherit border-0 border-b-2 border-black border-solid cursor-pointer"
                                onClick={handleAboutClick}
                            > {location.pathname === '/' ? 'About' : 'Home'}</button>
                            <button onClick={() => navigate('/Signin')} className="text-black font-inter font-bold text-xl bg-inherit border-none cursor-pointer">Login</button>
                        </div>
                    </div>
                )}
            </div>
